 ...
 */
function apf_render_filter_form( $preset_id = null ) {
    // Load preset data if a preset is provided
    $preset = $preset_id !== null ? apf_get_filter_preset( $preset_id ) : null;
    $selected_category = $preset['category'] ?? '';

    // Fetch product categories
    $categories = apf_get_product_categories();
    ?>
    <form id="apf-filter" method="post">
        <div>
            <label for="product_category"><?php _e( 'Category', 'ajax-product-filter' ); ?></label>
        ...
        <button type="submit"><?php _e( 'Apply Filter', 'ajax-product-filter' ); ?></button>
    </form>
    <?php
}
...
function apf_ajax_filter_products() {
    ...
    if ( $query->have_posts() ) {
        ...
        echo '<p>' . __( 'No products found.', 'ajax-product-filter' ) . '</p>';
    }
    wp_reset_postdata();
